...
router.get("/audit-logs", requireAuth, requireRole("admin"), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    res.json({
      success: true,
      data: { logs: logs.map((l) => ({ id: l.id, userId: l.userId, action: l.action, resource: l.resource, resourceId: l.resourceId, details: l.details, ipAddress: l.ipAddress, userName: l.user?.name ?? null, email: l.user?.email ?? null, createdAt: l.createdAt.toISOString() })), total, page },
    } satisfies ApiResponse);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
router.get("/embeddings/status", requireAuth, requireRole("admin"), async (_req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const companies = await db.company.count({ where: { company_json: { isNot: null } } });
...
router.post("/users/:userId/impersonate", requireAuth, requireRole("admin"), async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const targetUser = await db.user.findUnique({ where: { userId: targetUserId } });
    if (!targetUser) {
      res.status(404).json({ success: false, error: "User not found" } satisfies ApiResponse);
      return;
    }

    const token = generateJWT(targetUser.userId, targetUser.email, targetUser.role);
