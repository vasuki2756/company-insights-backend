...
router.get("/dashboard", requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const [user, totalCompanies, targetCompanies, userSkills, companyReqCount] = await Promise.all([
      ...
      db.company.count(),
      db.studentTarget.findMany({ where: { userId: req.user.sub }, include: { company: true } }),
      db.studentSkill.findMany({ where: { userId: req.user.sub }, include: { skill: true } }),
      db.company_skill_levels.count(),
    ]);
    ...
    const userSkillMap = new Map(userSkills.map((us) => [us.skillSetId, us.proficiencyLevel]));
    const scored = await Promise.all(
      targetCompanies.map(async (tc) => {
        const reqs = await db.company_skill_levels.findMany({ where: { company_id: tc.companyId } });
        const matchCount = reqs.filter((r) => (userSkillMap.get(r.skill_set_id) ?? 0) >= r.required_level).length;
        const score = reqs.length > 0 ? Math.round((matchCount / reqs.length) * 100) : 50;
        return { id: tc.company.company_id, name: tc.company.name, category: tc.company.category, score };
      }),
    );
    ...
    const payload = {
      ...
      targets: targetCompanies.map((tc) => ({
        ...
        addedAt: tc.createdAt.toISOString(),
      })),
      profileData,
    };
...
router.get("/recommendations", requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const user = await db.user.findUnique({ where: { userId: req.user.sub } });
    ...
    const companies = await db.company.findMany({
      include: {
        company_skill_levels: { include: { skill_set_master: true } },
      },
    });
    ...
    const scored = companies.map((c) => {
      ...
      if (c.company_skill_levels.length > 0) {
        const matched = c.company_skill_levels.filter((r) => (userSkillMap.get(r.skill_set_id) ?? 0) >= r.required_level).length;
        score += Math.round((matched / c.company_skill_levels.length) * 20);
        reasons.push(`${matched}/${c.company_skill_levels.length} skills matched`);
      }
      ...
      return {
        id: c.company_id, name: c.name, category: c.category, companyType: c.companyType,
...
router.post("/targets", requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const company = await db.company.findUnique({ where: { company_id: parsed.data.companyId } });
    if (!company) { res.status(404).json({ success: false, error: "Company not found" } satisfies ApiResponse); return; }

    const existing = await db.studentTarget.findFirst({
      where: { userId: req.user.sub, companyId: parsed.data.companyId },
    });
    if (!existing) {
      await db.studentTarget.create({
        data: { userId: req.user.sub, companyId: parsed.data.companyId },
      });
    }
...
router.delete("/targets/:companyId", requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    await db.studentTarget.deleteMany({ where: { userId: req.user.sub, companyId } });
    await invalidateCache(req.user.sub);
    res.json({ success: true } satisfies ApiResponse);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
router.get("/targets", requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const targets = await db.studentTarget.findMany({
      where: { userId: req.user.sub },
      include: { company: { select: { company_id: true, name: true, category: true, companyType: true, package: true, minCgpa: true, glassdoorRating: true, headquarters: true } } },
      orderBy: { createdAt: "desc" },
    });

    res.json({ success: true, data: { companies: targets.map((t) => ({ ...t.company, addedAt: t.createdAt.toISOString() })), count: targets.length } } satisfies ApiResponse);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
router.get("/favorites", requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const favorites = await db.studentTarget.findMany({
      where: { userId: req.user.sub },
      include: { company: { select: { company_id: true, name: true, category: true, package: true, minCgpa: true, glassdoorRating: true, headquarters: true } } },
      orderBy: { createdAt: "desc" },
    });

    res.json({ success: true, data: { companies: favorites.map((f) => ({ ...f.company, addedAt: f.createdAt.toISOString() })), count: favorites.length } } satisfies ApiResponse);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
router.put("/prep-progress", requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    await db.studentSkill.upsert({
      where: { userId_skillSetId: { userId: req.user.sub, skillSetId: parsed.data.skillId } },
      create: { userId: req.user.sub, skillSetId: parsed.data.skillId, proficiencyLevel: parsed.data.level },
      update: { proficiencyLevel: parsed.data.level },
    });

    const requirements = await db.company_skill_levels.findMany({
      where: { skill_set_id: parsed.data.skillId },
      select: { required_level: true },
    });

    const maxRequired = Math.max(...requirements.map((r) => r.required_level), 0);
...
router.get("/prep-progress", requireAuth, async (req: AuthenticatedRequest, res: Response): Promise<void> => {
  try {
    ...
    const targetIds = (await db.studentTarget.findMany({
      where: { userId: req.user.sub },
      select: { companyId: true },
    })).map((t) => t.companyId);

    const requirements = targetIds.length > 0
      ? await db.company_skill_levels.findMany({ where: { company_id: { in: targetIds } } })
      : [];

    const skills = userSkills.map((us) => {
      const reqs = requirements.filter((r) => r.skill_set_id === us.skillSetId);
      const maxRequired = Math.max(...reqs.map((r) => r.required_level), 0);
      const gap = Math.max(0, maxRequired - us.proficiencyLevel);
      return {
        skillId: us.skillSetId, skillName: us.skill.skill_set_name, category: us.skill.category,
        currentLevel: us.proficiencyLevel, requiredLevel: maxRequired, gap,
        gapHours: gap * 10, progress: us.proficiencyLevel,
      };
    });
