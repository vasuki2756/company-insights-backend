...
export async function answerCompanyQuestion(
  ...
  const prompt = buildCompanyQuestionPrompt(company.name, question, profileText);
...
export async function skillGapAnalysis(
  ...
  const studentSkillMap = new Map(
    student.studentSkills.map((s) => [s.skill.name.toLowerCase(), s.currentLevel]),
  );
  ...
  const gaps: SkillGapItem[] = requirements.map((req) => {
    ...
    const gap = Math.max(0, req.requiredLevel - userLevel);
  ...
  const studentSkillsText = student.studentSkills
    .map((s) => `${s.skill.name}: Level ${s.currentLevel}`)
    .join("\n");

  const companyRequirementsText = requirements
    .map((r) => `${r.skill.name}: Required Level ${r.requiredLevel} (${r.criticality})`)
    .join("\n");

  const prompt = buildSkillGapPrompt(studentSkillsText, companyRequirementsText, company.name);
...
export async function generateInterviewPrepQuestions(
  ...
  const techStack = company.profile?.techStack ?? [];
  const companyDescription = company.profile?.overviewText ?? `${company.name} is a ${company.category ?? ""} company.`;
